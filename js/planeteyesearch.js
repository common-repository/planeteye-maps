...
var map;
var searcher;
var host = "http://localhost:81"; //"http://www.planeteye.com";
...
function linkToPlace(customerid, blogurl) {
	   // return the details in the 'additional information' box...
		...
		var unescapedurl = $('#locationwebsite').val();
		var usePlanetEyeLink = $(".additionalbody input:checked").length > 0 ? true : false;
			...
			type: "GET",
				...
				success: function(obj, status) {
						if (obj != null && obj.result != null && (obj.result.url != null || obj.result.url != '')) {
							if (!usePlanetEyeLink && url && url.length > 3) {
								insertLink(url, obj.result.title, obj.result.description, obj.result.latitude, obj.result.longitude, obj.result.address, obj.result.placeId);
							} else {
								insertLink(obj.result.url, obj.result.title, obj.result.description, obj.result.latitude, obj.result.longitude, obj.result.address, obj.result.placeId);
							}
						} else {
							// if no link was found, insert a link to the search page.
							insertLink(host + "/Search.aspx?refcon=expertblog&q=" + name + "," + address, name, "", lat, lng, address, -1);
						}
				},
...
function insertLink(link, title, desc, lat, lng, address, planetEyePlaceId) {
	...
	if (!encodeLink) {
		...
		if (elm == null) {
			...
			for (i=0; i<elementArray.length; i++) {
				setAllAttribs(elm = elementArray[i], encodeLink, encodeTitle, desc, lat, lng, address, planetEyePlaceId);
			}
		} else {
			setAllAttribs(elm, encodeLink, encodeTitle, desc, lat, lng, address, planetEyePlaceId);
		}	
...
function setAllAttribs(elm, href, title, desc, lat, lng, address, planetEyePlaceId) {
...
//        var href = formObj.href.value;
	...
	// ensure we include the planeteye link somewhere
	if (planetEyePlaceId > 0) {
        	setAttrib(elm, 'rel', 'planeteye-place.' + planetEyePlaceId);
	} else {
        	setAttrib(elm, 'rel', 'planeteye-place.-1');
	}
